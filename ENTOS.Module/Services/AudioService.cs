...
namespace ENTOS.Module.Services 
{
    ...
    public partial class AudioService : BaseService
    {
        ...
        #region SourceCode3309ImportCode
        ...
        {
            string fileName = $"{item.Start.ToString(@"dd\.hhmmss")}{System.IO.Path.GetExtension(item.FileName)}";
            ...
            //lock (locker)
            //{
            //    stringBuilder.AppendLine(string.Format(trainFormat, fileName, item.ContentText));
            //    index++;
            //    Tools.ShowOrCloseDefaultWaitForm(null, $"{index.ToString("D")}/{total.ToString("D")}", stopWatch.Elapsed, true);
            //}
        }
        private void FfmpegAudioConverter(ref string fileName, string fullPath, string wavFolder, TimeSpan startTime)
        {
            ...
            if (!fileName.EndsWith(targetExt, StringComparison.OrdinalIgnoreCase))
            {
                ...
                Module.Helpers.ProcessHelper.RunProcessOutside("ffmpeg", args);
        ...
        private void WriteToTrainFile(IList<string> lines, string saveFolder)
        {
            ...
            // Shuffle nếu cần (để tránh bias nếu dữ liệu theo thứ tự)
        ...
        #endregion SourceCode3309ImportCode
        ...
        private bool EndContentIsBreakLine(Audio audio)
        {
            ...
            if (content.EndsWith('.'))
                return true;
            foreach (var endText in Module.Helpers.TextHelper.NewLineText)
