...
{    
    public partial class ConsumeFilterFolderViewController : ViewController<ListView>
    {
        ...
        //    }
        //}
        public ConsumeFilterFolderViewController()
        {
            filteringCriterionAction = new SingleChoiceAction(
                    this, "ConsumeFilterFolderCriterion", PredefinedCategory.Filters);
            ...
            TargetObjectType = typeof(Module.BusinessObjects.Consume);
        ...
        protected override void OnViewControlsCreated()
        {
            ...
            if (listview != null)
            {
                //Controller này chỉ áp dụng cho Popup ListView hoặc LookupListView
                if (View.Id.Split("_").Count() > 2)
                    return;
                ...
                if (filteringCriterionAction.SelectedItem == null)
                {
                    var criteria = CriteriaOperator.Parse("FolderType = 'Consume' and UpperFolder.FolderType <> 'Consume'");
                    
                    CreateDefaultFilter(criteria);
        ...
        private void CreateDefaultFilter(CriteriaOperator criteria)
        {
            ...
            //if (View.ObjectSpace.Owner is DevExpress.ExpressApp.ListView)
            //{
                //var ownerListView = (DevExpress.ExpressApp.ListView)View.ObjectSpace.Owner;
                //Trường hợp lọc theo nạp đối tượng
                //var objectTypeCriteria = CriteriaOperator.Parse("ObjectType = ?", ownerListView.CollectionSource.ObjectTypeInfo.Type);
                //criteria = CriteriaOperator.Or(criteria, CriteriaOperator.Parse("UpperFolder.ObjectType <> ?", ownerListView.CollectionSource.ObjectTypeInfo.Type));
                //criteria = CriteriaOperator.And(criteria, objectTypeCriteria);
            //}
            var listFolder = ObjectSpace.GetObjects<Module.BusinessObjects.Folder>(criteria);
            if(listFolder is null || listFolder.Count == 0)
            {
                listFolder = ObjectSpace.GetObjects<Module.BusinessObjects.Folder>(baseCriteria);
            }
