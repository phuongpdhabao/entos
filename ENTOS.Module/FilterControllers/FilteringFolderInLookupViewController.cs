...
{    
    public partial class IFilterFolderLookupViewController : ViewController
    {
        ...
        public IFilterFolderLookupViewController()
        {
            filteringCriterionAction = new SingleChoiceAction(
                    this, "IFilterFolderLookupViewCriterion", PredefinedCategory.PopupActions);
        ...
        protected override void OnViewControlsCreated()
        {
            ...
            if (listview != null && listview.Id.EndsWith("_LookupListView"))
            {
                //Chỉ áp dụng nếu lọc theo Type
                var typeName = View.ObjectTypeInfo.Type.Name;
                object currentFolderType = null;
                
                //Nếu không tìm thấy loại thì bỏ qua
                if(!System.Enum.TryParse(typeof(Module.BusinessObjects.SoftwareObjectType), typeName, out currentFolderType))
                    return;
                ...
                if (filteringCriterionAction.SelectedItem == null)
                {
                    CreateDefaultFilter(CriteriaOperator.Parse("FolderType = ? and UpperFolder.FolderType <> ?", currentFolderType, currentFolderType));                   
                    //Lọc by Type thì sẽ không có lựa chọn ban đầu
                    //if (filteringCriterionAction.Items.Count > 0)
                    //{
                    //    if (filteringCriterionAction.SelectedItem == null)
                    //    {
        ...
        private string _dataCriteria = null;
        private string GetDataCriteria()
        {
            if (string.IsNullOrEmpty(_dataCriteria))
            {
                _dataCriteria = "[Folder.Oid] = ?";
                foreach (CustomFilter customAttribute in View.ObjectTypeInfo.Type.GetCustomAttributes<CustomFilter>())
                {
                    ...
                    {
                        _dataCriteria = customAttribute.Criteria;
                        break;
                    }
                }
            }
            return _dataCriteria;
        }
        ...
        private void CreateDefaultFilter(CriteriaOperator criteria)
        {
            ...
            foreach (var folder in listFolder)
            {
                ...
                {                   
                    CreateTreeSource(null, folder, GetDataCriteria(), "");
                }
        ...
        private void CreateTreeSource(ChoiceActionItem parentItem, Module.BusinessObjects.Folder currentItem, string data, string prefix)
        {
            if (currentItem != null && !string.IsNullOrEmpty(data))
            {
                ...
                if (foundItem == null)
                {
                    //Chuyển sang khi nào dùng thì tạo điều kiện lọc
            ...
            object sender, SingleChoiceActionExecuteEventArgs e)
        {
            ...
            {
                data = AddAllChildCriteriaOperator(null, data).LegacyToString();
            }
